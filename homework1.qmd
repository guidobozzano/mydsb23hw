...
#Based on the wind speed distribution I can observe outliers when the wind speed is exactly 11 as there are no observations with that information. The same happens when the wind speed is 1 or 2.

ggplot(july13, aes(x=dewp, y=humid)) +
  geom_point() +
  labs(title = "Relationship between Dewp and Humid variables")

july13 %>% 
  summarise(corr_dh=cor(dewp,humid))

#There seems to be a positive relationship between humidity and dew point temperature since the coefficient of correlation is 0.53. Therefore, as the humidity in the air increases the dew point temperature also increases

ggplot(july13, aes(x=precip, y=visib)) +
  geom_point() +
...
#On the other hand the relationship between visibility and precipitation is not that clear as the coefficient of correlation is -0.24. This indicates that there is not clear linear relationship between the precip and visib variables
...
answer <- left_join(planes, flights, by="tailnum", na_matches = "never")
...
To make a graph that looks like the one above I believe I should create a ggplot that facet wraps by origin and carrier. The plot per each wrap would be cancellation by month.
